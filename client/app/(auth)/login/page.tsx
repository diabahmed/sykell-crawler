...
export default function LoginPage() {
  ...
  const handleLogin = async (values: LoginFormValues) => {
    setIsLoading(true);
    try {
      // The login endpoint sets the cookie, but we don't get user data back.
      const response = await apiClient.post("/auth/login", values);
      login(response.data.user); // This will fetch the user data and set it in the store
      ...
      // We only want to set loading to false if there's an error,
      // otherwise the page will be reloading.
      setIsLoading(false);
    }
  };
        ...
        <div className="w-full relative max-w-5xl overflow-hidden flex flex-col md:flex-row shadow-xl">
          ...
          <div className="bg-black text-white p-8 md:p-12 md:w-1/2 relative rounded-bl-3xl overflow-hidden">
            <h1 className="text-2xl md:text-3xl font-medium leading-tight z-10 tracking-tight relative">
              Welcome Back to Sykell Crawler.
            </h1>
          </div>
